...
def getImageCorners(image):
    ...
    corners[1][0] = np.array([image.shape[x], 0])
    corners[2][0] = np.array([image.shape[x], image.shape[y]])
    corners[3][0] = np.array([0, image.shape[y]])
...
def findHomography(image_1_kp, image_2_kp, matches):
    ...
    for mat in matches:
        pts1.append(image_1_kp[mat.queryIdx].pt)
        pts2.append(image_2_kp[mat.trainIdx].pt)

    image_1_points = np.asarray(pts1, dtype=np.float32)
    image_2_points = np.asarray(pts2, dtype=np.float32)

    M_hom, inliers = cv2.findHomography(image_1_points,
                                        image_2_points,
                                        cv2.RANSAC,
                                        5.0)
...
def blendImagePair(warped_image, image_2, point):
    ...
    image_2_copy = np.copy(image_2)
    image_2_expanded = np.zeros_like(warped_image)
    # REPLACE THIS WITH YOUR BLENDING CODE.
    """# Feathering attempt
    image_2_expanded[point[1]:point[1] + image_2.shape[0],
                 point[0]:point[0] + image_2.shape[1]] = image_2
    # Create step function
    step_x = np.zeros_like(warped_image)
    step_y = np.zeros_like(warped_image)
    step_x[: ,:point[0]] = 255
    step_y[:point[1], :] = 255

    step = step_x + step_y
    step[step > 255] = step[step > 255]*0.5

    # Blur step function
    blurred_step = cv2.GaussianBlur(step, (75,75), 0)

    blurred_step = blurred_step*0.00392157 # 1/255

    output_image = output_image*blurred_step

    image_2_expanded = image_2_expanded*(1 - blurred_step)

    output_image = output_image + image_2_expanded
    """

    # Average of overlap
    warped_overlap = output_image[point[1]:warped_image.shape[0],point[0]:warped_image.shape[1]]
    image_2_overlap = warped_overlap[:image_2.shape[0], :]

    boolean_overlap = image_2_overlap > 0

    output_image[point[1]:warped_image.shape[0],point[0]:warped_image.shape[1]] = \
        warped_image[point[1]:warped_image.shape[0], point[0]:warped_image.shape[1]]*0.5

    image_2_copy[boolean_overlap] = image_2_copy[boolean_overlap]*0.5

    image_2_expanded = np.zeros_like(warped_image)

    image_2_expanded[point[1]:point[1] + image_2_copy.shape[0],
                 point[0]:point[0] + image_2_copy.shape[1]] = image_2_copy
...
def warpImagePair(image_1, image_2, homography):
    ...
    transformed_corners_1 = cv2.perspectiveTransform(corners_1, homography)

    transformed_corners = np.concatenate((transformed_corners_1,
                                          corners_2))

    min = transformed_corners.min(axis=0)
    max = transformed_corners.max(axis=0)
    ...
    translation_mat = np.array([[1, 0, -1 * x_min],
                                [0, 1, -1 * y_min],
                                [0, 0, 1]])

    translated_hom = np.dot(translation_mat, homography)

    vector = (x_max - x_min, y_max - y_min)

    warped_image = cv2.warpPerspective(image_1, translated_hom, vector)
